...
# Usage:
#
# ./eval_sick_ablation.sh <ncores> <option> <split> <templates.yaml>
#
# Example:
#
# ./eval_sick_ablation.sh 10 tsurgeon train semantic_template_event.yaml
#
...
for f in ${plain_dir}/sick_${dataset}_*.answer; do
  let total++
  base_filename=${f##*/}
  sys_filename=${results_dir}/${base_filename/.answer/.txt}.${option}.answer
...
<tr>
  ...
  <td>EasyCCG</td>
  <td>depccg</td>
</tr>" > $results_dir/main_sick.${option}.html
...
for gold_filename in `ls -v ${plain_dir}/sick_${dataset}_*.answer`; do
  ...
  system_filename=${results_dir}/${base_filename/.answer/.txt}.${option}.answer
  gold_answer=`cat $gold_filename`
  echo '<tr>
  <td>'${base_filename/.answer/}'</td>
  <td>'${sentences}'</td>
  <td>'$gold_answer'</td>' >> $results_dir/main_sick.${option}.html
  for parser in "" "candc." "easyccg." "depccg."; do 
    if [ -e ${results_dir}/${base_filename/.answer/.txt}.${option}.${parser}answer ]; then
    ...
    echo '<td><a style="background-color:'$color';" href="'${base_filename/.answer/.txt}.${option}.${parser}html'">'$system_answer'</a>' >> $results_dir/main_sick.${option}.html
  done
  echo '</tr>' >> $results_dir/main_sick.${option}.html
done
...
echo -e "Multi-parsers:" > ${results_dir}/score.${option}.txt
python ${ccg2lambda}/report_results_sick.py gold.results system.results >> ${results_dir}/score.${option}.txt
echo -e "C&C:" >> ${results_dir}/score.${option}.txt
python ${ccg2lambda}/report_results_sick.py gold.results candc.results >> ${results_dir}/score.${option}.txt
echo -e "EasyCCG:" >> ${results_dir}/score.${option}.txt
python ${ccg2lambda}/report_results_sick.py gold.results easyccg.results >> ${results_dir}/score.${option}.txt
echo -e "depccg:" >> ${results_dir}/score.${option}.txt
python ${ccg2lambda}/report_results_sick.py gold.results depccg.results >> ${results_dir}/score.${option}.txt

cat ${results_dir}/score.${option}.txt
