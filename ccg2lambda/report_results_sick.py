...
def get_fracas_info(problem_id):
    ...
    """
    info = re.findall(r'sick_(.+)_(\d+)', problem_id)
    assert info, 'Correct SICK ID information could not be retrieved for {0}' \
      .format(problem_id)
    section, fracas_num  = info[0]
...
def load_answers(finput_filename):
    ...
    for line in finput:
        ...
        problem_id = line_splitted[0] # E.g. sick_001_train
